...
function get_conversion_rates() {
    return {
        ...
        sp_gp: game.settings.get("5e-custom-currency", "sp-gp"),
...
function alterTradeWindowCurrency(html) {
    ...
    ['pp', 'gp', 'sp', 'cp'].forEach(dndCurrency => {
...
function alterPartyOverviewWindowCurrency(html) {
    ...
    $(currencies[2]).text(altNames["spAlt"])
    $(currencies[3]).text(altNames["cpAlt"])
    $(currencies[4]).text(`${altNames["gpAlt"]} (${game.i18n.localize('party-overview.TOTAL')})`)
}